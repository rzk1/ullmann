...
&=  E_{GS} - k_BT\ln{w_{GS}} \\
&\quad - k_BT\ln{\left[\frac{\sqrt{\pi}}{\sigma} \left(\frac{8\pi^2k_BT}{h^2} \right)^{\frac{2}{3}}(I_aI_bI_c)^{\frac{1}{2}} \right]} \\
&\quad -k_BT\ln{\left[ \left( \frac{P}{k_BT} \right) \left( \frac{2\pi Mk_BT}{h^2}\right)^{-\frac{3}{2}}  \right]}
\end{split}
\end{equation}
...
For the intermediates adsorbed on surface, the vibration and rotation vanish since surface defines their motion into 2D mode. The translational partition function also adopts a 2D forms~\cite{ullmann_172}:
\begin{equation}
\begin{split}
A &= -\frac{k_BT}{N}\ln{(q_{\text{elec}}q_{\text{tran}})} \\
&=  E_{GS} - k_BT\ln{w_{GS}} \\
&\quad -k_BT\ln{\left[ \rho \left( \frac{2\pi Mk_BT}{h^2}\right)^{-1}  \right]}
\end{split}
\end{equation}
...
\caption{Energy profile of the Ullmann reaction of bromobenzene on Cu(111) after applying entropy effects following ideal surface pathway. Levels shown with black color are states without additional entropy-related functions, whereas the dimgrey states are with a entropy correction at \SI{200}{\kelvin} temperature and grey states are at \SI{400}{\kelvin} temperature. In both two conditions, entropy correction are completed at \SI{e-10}{\milli\bar} UHV condition, and consistent with the reaction condition for Ullmann process~\cite{ullmann_147, ullmann_173, Ullmann_57}. The concentration of molecule is approximately 0.8 molecule per \si{\nano\metre\squared}, and it is verified by STM image~\cite{ullmann_67}}.

...
\begin{abstract}
%RZK-ignore: the major problem with the new abstract is that it does not clearly answer the question it asks.
...
%RZK: shorten the list [such as one- and two-dimensional cojugated polymers]
Despite substantial progress in determining the mechanism of this reaction, one fundamental question that still remains unanswered is whether the coupling is catalyzed directly by surface atoms or adatoms.
In this work, the feasibility of the adatom creation and adatom-catalyzed Ullmann coupling of  iodo-, bromo- and chlorobenzene on Cu(111), Ag(111) and Au(111) surfaces is examined using [density functional theory] DFT modeling. 
%In this work, the feasibility of the adatom creation and [their] role in the Ullmann coupling of iodo-, bromo- and chlorobenzene on the Cu(111), Ag(111) and Au(111) surfaces is examined using [density functional theory] DFT modeling. 


In this work, the feasibility of the adatom creation during the Ullmann coupling of two phenyl groups, formed from monohalogenated benzenes, their adatom-catalyzed Ullmann coupling on Cu(111), Ag(111) and Au(111) surfaces using DFT modeling. 

The phenyl group, which is produced in the dehalogenation of monosubstituted benzenes, 



Analysis of competing pathways reveals that two phenyl groups extract a silver atom from Ag(111) surface faster ($\Delta E^{\ddagger} = 0.43$~eV) than they form the carbon-carbon bond ($\Delta E^{\ddagger} = 0.62$~eV). However, on Cu(111) and Au(111), the extraction process is slower ($\Delta E^{\ddagger} = 0.71$~eV Cu, $0.36$~eV Au) than the ideal-surface C--C formation ($\Delta E^{\ddagger} = 0.49$~eV Cu, $0.14$~ev Au). 
%RZK: this version below not mention phenyl groups. In fact earlier changes remove the notion that it is the organic groups that extract the adatom. Because of this it is not clear later why the phenyl-adatom bonds are mentioned later. 
%Analysis of competing pathways reveals that the adatom creation on Ag(111) [assisted by phenyl groups] ($\Delta E^{\ddagger} = 0.43$~eV) is faster than the [ideal-surface] C-C bond formation  ($\Delta E^{\ddagger} = 0.62$~eV). However, on Cu(111) and Au(111), the extraction process is slower ($\Delta E^{\ddagger} = 0.71$~eV Cu, $0.36$~eV Au) than the C–C bond formation ($\Delta E^{\ddagger} = 0.49$~eV Cu, $0.14$~ev Au). 

The surprising ease of the phenyl-assisted adatom creation on Cu(111) and Ag(111) ($\Delta E = 0.16$~eV Cu, $0.14$~eV Ag) is a result of the significant strengthening of the phenyl-metal bonds upon the extraction. However, these strong phenyl-adatom bonds create an insurmountable barrier for the subsequent adatom-catalyzed coupling step on both metals ($\Delta E^{\ddagger} = 1.78$~eV Cu, $1.52$~eV Ag).

The adatom creation is facilitated by the strengthening carbon-metal bonds upon the extraction. However, these strong bonds create an insurmountable barrier for the subsequent adatom catalyzed C-C coupling on Cu(111) and Ag(111) ($\Delta E^{\ddagger} = 1.78$~eV Cu, $1.52$~eV Ag). 

In contrast, Au adatoms do not bind phenyl groups strongly and[, if they already exist on the Au(111) surface,] can catalyze the C--C bond formation almost as efficiently ($\Delta E^{\ddagger} = 0.20$~eV) as surface atoms ($\Delta E^{\ddagger} = 0.14$~eV).
...
%-A comprehensive analysis of the computed data reveals several trends that can facilitate design of efficient adatom-catalyzed bond-forming processes.
The revealed trends can facilitate design of efficient on-surface reactions.

\textbf{Keywords:} Ullmann coupling, adatom, defects, pi-conjugated two-dimensional polymers, surface reaction, metal catalysis, DFT.
\end{abstract}
...
Unique and tunable properties of $\pi$-conjugated nanomaterials such as graphene nanoribbons~\cite{ullmann_106, ullmann_45, ullmann_107, ullmann_101} and two-dimensional conjugated organic polymers make them promising candidates for a variety of electronic and optoelectronic devices~\cite{ullmann_113, ullmann_114}. Practical applications of these materials in devices requires scalable methods to create extended structures with low defect density.
...
The Ullmann coupling of aryl halides on metal surfaces -- copper, silver and gold -- is currently the most promising bottom-up strategy to assemble atomically precise $\pi$-conjugated carbon nanostructures with high degree of control over their electronic properties~\cite{ullmann_33, ullmann_42, ullmann_43, ullmann_45, ullmann_46,ullmann_47, ullmann_140, ullmann_148, ullmann_149, ullmann_150, ullmann_98, ullmann_97, jacs2016, ullmann_152}. 
...
%RZK: We need to discuss references in the sentence above. Can we say "created in the process of on-surface ULLMANN reactions"? Are these references about Ullmann? Why don't we discuss them in detail below? Can we say "growing body of EXPERIMENTAL evidence"?
...
For example, DFT modeling shows that two fluoranthenyl groups lift their shared gold atom \SI{2.2}{\angstrom} above its ideal surface position, which is noticeably higher than the \SI{0.5}{\angstrom} raise produced by a single group~\cite{jpcc2018} and than the \SI{1.4}{\angstrom} radius of Au atom.
...
Levels shown with bright colors are states with bromine as the halogen, whereas the faint upper and lower levels denote states with chlorine and iodine as the halogen, respectively. It is assumed that halogen atoms do not influence the reaction after the dehalogenation step and, therefore, only the energy levels for bromine containing species are shown. The numerical values of all species are collected in Tables~\ref{SI-table:bondlength}, \ref{SI-table:idealsurface} and~\ref{SI-table:adatom-longitude}.}
...
In the initial steps of the Ullmann reaction (Fig.~\ref{fig:completeenergy}) the monohalogenated benzene molecules, physisorbed on the metal surface (\textbf{PHYS}), dissociate with the formation of the surface-bound phenyl groups (\textbf{DHAL}), which then combine forming organometallic bridge structures (\textbf{CMC}).
...
The ideal-surface path has a relatively small \SI{0.49}{\electronvolt} energy barrier, which is in qualitative agreement with previous calculations~\cite{pccp2010, jacs2013}. %, which in line with the onset temperature for the C--C bond formation from two phenyl radicals on Cu(111) is \SI{300}{\kelvin} in experiment~\cite{sur_sci01}.
...
%REVIEW: It is also worth noting that the adatom extraction assisted by two phenyl groups requires less energy than the previously described extraction assisted by a single phenyl group and a halogen atom, which may happen immediately after the dehalogenation step~\cite{chemeurope2017}. In the previous study~\cite{chemeurope2017}, which used smaller slab models and different density functional, the energy of the extraction of a copper atom assisted by phenyl and iodine was calculated to be \SI{0.88}{\electronvolt} (cf. \SI{0.16}{\electronvolt}  phenyl-assisted extraction here). %{\comm RZZK0529: Any explanations of why iodine is less helpful?} {\zhzh ZZ: I did not see their explanation on this. My idea: During the deiodination, the largest height that copper atom involved can be lifted is \SI{0.15}{\angstrom}, which means that the interaction between the copper atom and one phenyl radical with one iodine atom is weaker than two phenyl radicals.}
...
It is worth noting that the adatom pathway can also be viewed as a sum of two different steps: the highly exothermic ideal-surface C--C bond formation (\SI{-2.00}{\electronvolt}) and the energy demanding adatom extraction on clean surface (\SI{1.76}{\electronvolt}). 
It is therefore not surprising that the entire pathway is nearly thermoneutral (\SI{-0.24}{\electronvolt}). 
Interestingly, however, all three steps along the adatom pathway are also almost thermoneutral, with their own energy balancing mechanisms (Fig.~\ref{fig:completeenergy}). 
...
%RZK: There is not legend in the graph! Add it.
}
...
The collected data shows that the strengthening of the phenyl-adatom binding relative to the phenyl-surface binding (Fig.~\ref{fig:onlysurface}) is the main characteristic of the efficiency of the adatom catalysis. 
...
The example of Au(111) demonstrates that low strengthening leads to weak extraction assistance, which makes the extraction step uncompetitive. On the other hand, the examples of Ag(111) and Cu(111) show that high strengthening makes the extraction competitive but prohibits the second C--C bond formation step. 

To obviate the dilemma, other characteristics of the process must be considered when the surface is chosen or designed. It is reasonable to expect, that the phenyl-assisted extraction can be facilitated without hindering the adatom catalyzed C--C bond formation if metal catalysts with low energy of the unassisted adatom extraction are employed. % (green paths in Figs.~\ref{fig:completeenergy}, \ref{fig:Ag_all}, \ref{fig:Au_all}).
It turns out that it is possible to formulate a simple theoretical condition for competitive adatom catalysis (details in \sinfo) if both the energy of unassisted adatom extraction, an intrinsic property of a metal, and strengthening of the phenyl-adatom binding are considered. 
Furthermore, our DFT energies for the three metals can be used to estimate the boundaries of the competitive adatom-catalysis region in the space of these two parameters (Fig.~\ref{fig:conclusion}). 

Fig.~\ref{fig:conclusion} shows that a metal surface for a competitive adatom-catalyzed coupling of two phenyl groups must satisfy very stringent requirements. For example, Fig.~\ref{fig:conclusion} makes it obvious that the energy of the adatom extraction on a clean metal surface cannot exceed \SI{1.03}{\electronvolt} (cf. \SI{1.56}{\electronvolt} for gold). 
{\comm RZK-ignore: Include the following hypothetical examples?}
It can be speculated that alloys of copper, silver and gold with different ratio of the components can satisfy this requirement because the lattice incommensurabilities may make atom extraction easier. Metals with low cohesive energy can be considered as well. {\comm RZK-ignore: Any specific examples of such metals? Obviously, the should be able to catalyze the C--C formation.}

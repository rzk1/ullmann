...
As the data in Table.\ref{table:idealsurface} illustrates, final coupling step following ideal surface path (\textbf{CMC} to \textbf{DIM}) is highly exothermic, resulting in that biphenyl is the most thermodynamic stable species in along the RYK-black line in Fig.~\ref{fig:completeenergy}. In agreement with experiment, Ullmann coupling of bromobenzene end in biphenyl and bromine atoms adsoprbed on Cu(111) at \SI{350}{\kelvin}\cite{ullmann_67}. Then higher annealing temperature leads to the desorption of biphenyl and eventually bromine gas.
This path offers a relatively small energy barrier, which is line with onset temperature for C--C bond formation from two phenyl radicals on Cu(111) is \SI{300}{\kelvin} in experiment~\cite{sur_sci01}.

As shown in ~\ref{fig:bondformlift}, the raised cooper atom in this path migrates back step by step. with the tilt angle of phenyl ring with respect to surface varied from \SI{52}{\degree} to \SI{0}{\degree} in the end.
...
\begin{tabular}{ lccccccccc  }
 \hline
 \hline
 Distance & Halogen & \textbf{DHAL-2} & \textbf{CMC} & $\Delta$(\textbf{DIM$\ddagger$}$-$\textbf{CMC}) & \textbf{DIM$\ddagger$} & \textbf{DIM} & $\Delta$(\textbf{DIM}$-$\textbf{CMC}) & \textbf{DSRB} & \textbf{D-SURF}\\ 
 \hline 
...
Two possible orientations of adatom extraction have been explored in above steps, as shown in Fig.~\ref{fig:bondformadatom} with red labels and yellow labels, respectively. In red one, \textbf{X-CMC} moves to top direction with the adatom-vacancy pair forming in longitude, while it shifts to right with adatom-vacancy lying in latitude in yellow one, geometric and energetic data of former is shown in Table.~\ref{table:adatom-longitude}, latter in Table.~\ref{table:adatom-latitude}. The conversions in geometry along different configurations of longitude and latitude direction are similar, including the distance among two unsaturated carbons and the adatom. However, energy barrier of extraction in longitude direction is \SI{0.67}{\electronvolt} lower, which make it much more kinetically favorable than latitude direction. But extraction of adatom is endothermic in longitude and exothermic in latitude, lead intermediate \textbf{X-CMC} on right of vacancy becoming thermodynamically more stable, due to the stronger Van der Waals interaction between one perfectly covered phenyl ring and vacancy in this configuration. Second elementary steps -- formation of C--C bond on adatom are both exothermic for longitude and latitude directions, with longitude revealing a lower energy barrier. The stationary states \textbf{X-DIM-A} and \textbf{X-DIM-B} are closely stable. In \textbf{X-DIM-B}, newly formed adatom diffuses away and has no interaction with diphenyl product. The details of NEB energy of these two orientations can be viewed in Fig.~\ref{fig:distance-energy}, which clearly show that creating adatom in longitude direction will occur at a much higher speed than latitude.
...
In addition to the mechanism describe above, we explored an alternative reaction pathway directly from \textbf{X-CMC} to \textbf{X-DIM-B} instead of passing \textbf{X-DIM-A}. In this path, the formation of C--C bond and the diffusion of newly formed adatom away take place simultaneously, rather than one after the other in former path. The CI-NEB energy and geometry data is shown in \sinfo. Since this path displays an exceedingly high activation energy around (\SI{3.0}{\electronvolt}), and surpassing the previous energy barrier (\SI{1.38}{\electronvolt}) to a large scale, it seems not plausible to occur in nature.

...
DFT modeling is used to investigate the role of adatoms in the Ullmann coupling of monosubstituted benzenes on the commonly employed Cu(111) surface. It was found that the extraction of an adatom by two phenyl groups bonded to the same copper atom has only \SI{1000}{\electronvolt} is plausible and cannot be discarded -- competitive with C--C bond formation. The energetic cost of extracting the adatom is almost completely offset by the increased binding between the two phenyl groups and the adatom in the process of extraction. The binding strength is estimated to increase by (if compared to the binding to the surface atom).

These results suggest that adatoms should often be found in the CMC bridge structures and should be considered as an alternative in future studies. Is this true? Will the reaction simply proceed toward the normal product?

The increased binding strength makes the formation of the carbon-carbon bond between the two phenyl group extremely difficult. 
High barrier and no energy release.

The two phenyl groups bound to the extracted adatom can still participate in the coupling process if the adatom recombines with the vacancy -- the step that is found to have relatively low activation energy.

A simple discussion on pre-existing adatom affecting on-surface Ullmann reaction has also been present. 

%RZK0525. I am almost certain that this will not pay off: A 2D lattice model is proposed to complement entropy into theoretically potential energy, to explain a common lower energy barrier in C--C bond formation than dehalogenation in DFT calculation, which disagrees the temperature set in experiment.
...
%TWO follow-up projects appears to be useful. 

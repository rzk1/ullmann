...
%RZK: In Ullmann coupling, metals can serve as reactants or as catalysts.
%RZK: Cu or metal?
...
%RZK: it is unclear what Pd--Fe, Au--Pd means. Is it a range in the periodic table? Again, Ullmann==Cu?
...
% RZK: This looks like a very clear figure that explains the mechanism. Some modifications will be necessary LATER: 
...
%RZK: Is it better to use Scheme 2 from ullmann_29? Less details makes it easier to understand.
\label{fig:classical}
\end{figure}
...
%RZK: the last sentence only makes sence if classical coupling is limited to copper.
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.4\textwidth]{Fig/mechanism.png}
\caption{Elementary steps of a surface Ullmann Coupling reaction mechanism}
\label{fig_mecha}
\end{figure}

As shown in Fig.~\ref{fig_mecha}, the mechanism is also involved in the formation of organometallic intermediates in the process, which appears in the mechanism of solution Ullmann coupling reaction. The difference is the origin of metal in the intermediates. Since all reactants and intermediates are on the metal surfacel, it is apparently that the active metal species come directly from the surface atoms. However, the face of metal crystal is not a perfect, well-ordered surface in reality, there exits various of defect, adatoms, vacancies, as shown in Fig.~\ref{cyrstal_surface}~\cite{ullmann_49}. Figuring out how these imperfections affect the surface Ullmann coupling would consummate the utilization of various surface. Here we will mainly investigate how adatoms participate in surface Ullmann coupling reaction mechanism, details will be discussed in next section.
...
%RZZK: no reference to the figure steps (which figure steps?)
...
For $origin(1)$, it has been explored that the number of mobile adatoms on copper surface should be high at room temperature~\cite{ullmann_58}, especially near various surface defect. 
%(ZZ: did not finished)



In following section, the nature of metal atoms participating in each step of surface Ullmann coupling will be discussed. The discussion will also include whether the metal atoms are adatoms and the origin of adatoms if they are.

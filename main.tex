...
Atomic positions were optimized until the maximum force on atoms decreased below \SI{0.02}{\electronvolt/\angstrom}. 
{\comm RZK0525: Check the units of force in the previous sentence and below. Force is the energy divided by distance, not distance squared.} {\zhzh ZHZH0531: checked and clarified}
%
Transition state structures were located using the climbing-image nudged elastic band (NEB) with the VTST code~\cite{ullmann_59}. {\comm RZK0525: Did we really use the dimer method?} {\zhzh ZHZH0531: deleted, we did not use.}
%
In NEB calculations, an improved initial guess~\cite{ullmann_60, ullmann_99} for the minimum energy path was used and the positions of atoms were relaxed until the maximum force dropped below \SI{0.1}{\electronvolt/\angstrom}.

...
\begin{abstract}
%{\comm RZK0618: Mostly limited to Cu.}
...
The raise of this magnitude (cf. \SI{1.41}{\angstrom} radius of gold atom) is high enough to warrant further exploratory studies of the adatom creation.
...
\begin{table*}
\centering
\caption{Characterization of the intermediates along the ideal-surface pathway. %The states are labeled as in Fig.~\ref{fig:completeenergy}. 
...
{\comm RZK0628: Exchange the order of columns $\Delta$(\textbf{DIM$\ddagger$}$-$\textbf{CMC}) and \textbf{DIM$\ddagger$}. } {\zhzh ZZ: done}
}
...
%RZK0628: y-ticks placement is weird. There should be a tick mark at y=0. ZZ: done 
}
...
{\comm RZK0628: Use the energy of X-CMC in Figure~\ref{fig:onlysurface} to replace the 10000 marker in this paragraph.} {\zhzh ZZ:done}
...
It is suggested that the high energy cost of breaking multiple bonds between copper atoms during the phenyl-assited extraction is almost completely offset by the increased strength of binding of the phenyl groups to the extracted copper atom. This hypothesis is supported by the \SI{1.56}{\electronvolt} increase in the interaction energy between the two phenyl groups and the copper atoms after the extraction (Fig.~\ref{fig:onlysurface}). While it is difficult to separate this total interaction energy into the energy components describing $\pi$-metal and covalent carbon-adatom interaction, it is the covalent component that is most likely became stronger. It is unlikely that the $\pi$-metal interaction become stronger when the phenyl rings move farther away from the surface in \textbf{X-CMC} (Fig.~\ref{fig:distance-energy}). Another argument for the stronger covalent interactions is the \SI{0.12}{\angstrom} decrease in length of the carbon-adatom bond during the \textbf{CMC} to \textbf{X-CMC} transformation (Table~\ref{table:adatom-longitude}).
...
\begin{table*}
\centering
\caption{Characterization of the intermediates along the $[11\bar{2}]$ adatom pathway. %The states are labeled as in Fig.~\ref{fig:completeenergy}. 
The energies are relative to the \textbf{SURF} state. 
{\comm RZK0628: Exchange the order of columns $\Delta^{(b-a)}$ and \textbf{X-CMC$\ddagger$}$^{b}$. Exchange the order of columns  $\Delta^{(d-c)}$ and \textbf{X-DIM$\ddagger$}$^{d}$. The logic is: the state is introduced and only then referred to. Use this logic to correct table(s) in \sinfo.} {\zhzh ZZ:done}
}

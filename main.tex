...
%Do not use double-hyphen in the x and y-axis labels in (b). Single hyphen is ok.(done)
%Use the x-axis label from (b) in the y-axis label of (a).(done)
%The quality of (b) is poor. The resolution is too low.(done)
%In panel (a), label X-CMC datapoints with their y-values. In panel (b), label all points with their y-values.(done)
%In both panel, the y-axis should start at 0.0. Remove the negative portion.(done)
